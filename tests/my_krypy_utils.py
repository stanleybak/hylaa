...
def norm(x, y=None, ip_B=None, debug=False):
...
def hegedus(A, b, x0, M=None, Ml=None, ip_B=None):
    """Rescale initial guess appropriately (Hegedus trick).

    The Hegedus trick rescales the initial guess to :math:`\\gamma_{\\min} x_0`
    such that
...
class Arnoldi(object):
    ...
    def advance(self):
        ...
        if k == 1:
            print "krypy curvec norm5 = {}({})".format(numpy.linalg.norm(Av, 2), numpy.linalg.norm(Av, 2).hex())

        self.H[k+1, k] = norm(Av, ip_B=self.ip_B, debug=k==1)

        if k == 1:
            print "krypy norm = {}({})".format(self.H[k+1, k], self.H[k+1, k].hex())
