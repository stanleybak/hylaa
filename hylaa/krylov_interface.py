...
class KrylovInterface(Freezable):
    ...
    def arnoldi_parallel(start_pos, final_pos, numIter):
        ...
        assert KrylovInterface._is_loaded

        numInitVec = final_pos - start_pos + 1

        result_H = np.zeros((numInitVec * numIter * numIter))

        if not KrylovInterface._use_gpu:
            KrylovInterface._arnoldi_parallel_Cpu(start_pos, final_pos, numIter,
                                               result_H)
        else:
            KrylovInterface._arnoldi_parallel_Gpu(start_pos, final_pos, numIter,
                                               result_H)
    ...
    @staticmethod
    def getKeySimResult_parallel(dirMatrix_numRows, numInitVec, numIter,
                                 expm_column_list):
        ...
        if not KrylovInterface._use_gpu:
            KrylovInterface._getKeySimResult_parallel_Cpu(expm_column_mat,
                                                       keySimResult_tuples)

        else:
            KrylovInterface._getKeySimResult_parallel_Gpu(expm_column_mat,
                                                       keySimResult_tuples)
...
def test_getKeySimResult_parallel():
    ...
    KrylovInterface.load_keyDirSparseMatrix(keyDirMatrix_Sparse)
    print "number of Rows of keyDirMatrix is: {}\n".format(
        keyDirMatrix_numRows)
    ...
    for i in range(0, numInitVec):
        ...
        print "The {}-th exp(H*t) at time t = {} is: \n {}".format(
            i + start_pos, curTimeStep, expHt_tuples[i])

    keySimResult_tuples = KrylovInterface.getKeySimResult_parallel(
        keyDirMatrix_numRows, numInitVec, numIter, expHt_tuples)
